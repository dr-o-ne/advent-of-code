...
{
    public object Solve1(string input) => Solve(input, Convert1);

    public object Solve2(string input) => Solve(input, Convert2);

    private static long Solve(string input, Func<List<string>, List<long>> convertFunc)
    {
        long result = 0;

        List<Block> blocks = Parse(input);
        foreach (Block block in blocks)
        {
            List<long> values = convertFunc(block.Values);

            result += block.Operation == Operation.Add ?
                values.Aggregate(0L, (acc, x) => acc + x) :
    ...
    private static List<long> Convert1(List<string> input) =>
        input.Select(long.Parse).ToList();

    private static List<long> Convert2(List<string> input)
    {
        List<long> result = [];

        var values = input.ToList();
        var max = values.Max(x => x.Length);
    ...
    private static List<Block> Parse(string intput)
    {
        List<Block> result = [];
        string[] rows = intput.Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries);
        ...
        for (int i = 0; i < symbolIndexes.Count - 1; i++)
        {
            intervals.Add(new(symbolIndexes[i], symbolIndexes[i + 1] - 2));
        }

        int maxLength = rows.Select(x => x.Length).Max();
        intervals.Add(new(symbolIndexes[^1], maxLength - 1));
        ...
        foreach ((int start, int end) in intervals)
        {
            ...
            {
                temp.Add(rows[i].Substring(start, end - start + 1));
            }
            result.Add(new Block(temp, lastRow[start] == '+' ? Operation.Add : Operation.Multiply));
        }

        return result;
    }

    internal sealed record Block(List<string> Values, Operation Operation);
    ...
    [Fact] 
    public void Test1() => Assert.Equal(4277556L, Solve1(TestInput));

    [Fact]
    public void Test2() => Assert.Equal(6891729672676L, Solve1(Input));

    [Fact]
    public void Test3() => Assert.Equal(3263827L, Solve2(TestInput));

    [Fact]
    public void Test4() => Assert.Equal(9770311947567, Solve2(Input));
