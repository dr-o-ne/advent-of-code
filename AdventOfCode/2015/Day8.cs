...
{
    public int Solve1(string input) => Parse(input)
        .Sum(s => s.Length - DecodedLength(s));

    public int Solve2(string input) => Parse(input)
        .Sum(s => EncodedLength(s) - s.Length);
    ...
    {
        var cnt1 = s.Count(x => x == '"');
        var cnt2 = s.Count(x => x == '\\');

        return 2 + s.Length + cnt1 + cnt2;
    }
    ...
    [InlineData(1, @"""\x27""")]
    [InlineData(3, @"""a\\b""")]
    public void DecodedLengthTests(int expected, string s) => Assert.Equal(expected, DecodedLength(s));
    ...
    [InlineData(11, @"""\x27""")]
    public void EncodedLengthTests(int expected, string s) => Assert.Equal(expected, EncodedLength(s));

    [Fact]
    public void Test1() => Assert.Equal(1350, Solve1(Input));

    [Fact]
    public void Test2() => Assert.Equal(2085, Solve2(Input));

    private static IEnumerable<string> Parse(string input) => input
        .Split(Environment.NewLine);

    private const string Input = @"""qxfcsmh""
