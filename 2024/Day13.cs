...
public sealed class Day13
{
    ...
    private static long Solve(string input, long shift = 0) =>
        Parse(input, shift)
            .Select(Solve)
            .Where(x => x != NoSolution)
            .Sum();
    ...
    /*
        ...
        AY * i + BY * j = Y
    */
    private static long Solve(Equations eq)
    {
        long remainder = 0;
        var i = Math.DivRem(
            eq.Y * eq.BX - eq.X * eq.BY, 
            eq.AY * eq.BX - eq.BY * eq.AX, 
            out remainder
        );
    ...
    [Fact]
    public void Test1() => Assert.Equal(25751L, Solve1(Input));

    [Fact]
    public void Test2() => Assert.Equal(108528956728655, Solve2(Input));

    private static List<Equations> Parse(string input, long shift)
    {
        const string RegEx = @"X[+=](\d+),\s*Y[+=](\d+)";

        var result = new List<Equations>();

        var rawCases = Utils.ParseList<string>(input, Environment.NewLine)
            .Chunk(3);

        foreach (var rawCase in rawCases)
        {
            var match0 = Regex.Match(rawCase[0], RegEx);
            long ax = long.Parse(match0.Groups[1].Value);
            long ay = long.Parse(match0.Groups[2].Value);

            var match1 = Regex.Match(rawCase[1], RegEx);
            long bx = long.Parse(match1.Groups[1].Value);
            long by = long.Parse(match1.Groups[2].Value);

            var match2 = Regex.Match(rawCase[2], RegEx);
            long x = long.Parse(match2.Groups[1].Value);
            long y = long.Parse(match2.Groups[2].Value);

            result.Add(new Equations(ax, ay, bx, by, x + shift, y + shift));
        }

        return result;
    }

    internal sealed record Equations(
        long AX,
        long AY,
        long BX,
        long BY,
    ...
    private const string Input = @"Button A: X+63, Y+14
Button B: X+12, Y+37
