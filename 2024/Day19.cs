...
public sealed class Day19
{
    ...
    {
        (List<string> patterns, List<string> rows) = Parse(input);

        foreach (string row in rows)
            yield return CalculatePermutations(row, patterns, []);
    }

    private static long CalculatePermutations(string row, List<string> patterns, Dictionary<string, long> cache)
    {
        if(string.IsNullOrEmpty(row))
            return 1;

        if(cache.TryGetValue(row, out long value))
            return value;
        ...
        foreach (string pattern in patterns) {
            ...
            string rowRemainder = row[pattern.Length..];
            long count = CalculatePermutations(rowRemainder, patterns, cache);
            if (count > 0)
                cache[row] += count;
        }
